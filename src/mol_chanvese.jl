...
# ╔═╡ 9bdc6e39-bdf1-422c-bb39-81c723b46a09
using ModelingToolkit, Symbolics, Images, PlutoUI, Interpolations, DomainSets, CairoMakie, MethodOfLines

# ╔═╡ 66da8268-6ed6-4bd8-86ac-a593325accf4
TableOfContents()

# ╔═╡ 157cda5e-422e-4018-9a07-92247b431de4
begin
	@parameters x y t
	@variables ϕ(..)
end;

# ╔═╡ 266f2dfc-9ddc-4c81-ae4c-9b2905f8b7a0
begin
	Dt = Differential(t)
	Dx = Differential(x)
	Dy = Differential(y)
...
# ╔═╡ 79d0e508-361e-4d68-b4c1-8837fdda59b4
md"""
## Chan Vese Equation
[link](https://www.hindawi.com/journals/mpe/2013/508543/)

Energy Functional (Level Set Formulation)
```math
\begin{aligned}
F(c_1, c_2, \phi) &= \nu \int_{\Omega} |\nabla H(\phi) dxdy| + \int_{\Omega} (u^0 - c_1)^2 H(\phi) dxdy + \int_{\Omega} (u^0 - c_2)^2 (1 - H(\phi)) dxdy \\
\end{aligned}
\tag{1}
```
...
```math
\begin{aligned}
c_1 = \frac{\int_{\Omega} u^0 H(\phi) dx}{\int_{\Omega} H(\phi) dx}, \ \ 
c_2 = \frac{\int_{\Omega} u^0 (1-H(\phi)) dx}{\int_{\Omega} (1-H(\phi)) dx}
\end{aligned}
```
"""

# ╔═╡ a46729ee-2c35-4df3-b63a-2cb245e8bf4d
md"""
## Load Array(s)
"""

# ╔═╡ cf0c9e76-9e2c-4ce2-92b2-b569aeb5ee33
begin
	x_min, x_max = 1, 100
	y_min, y_max = 1, 100
end

# ╔═╡ 31e77f6f-223d-4d1f-8bf0-b30462a5ebb1
begin
	image = shepp_logan(x_max, y_max)
	image = convert(Array{Float64}, Gray.(image))
end;

# ╔═╡ 6c6c7b68-83c7-433e-bd3e-d811b5778d25
begin
	f(x, y) = sin(0.5 * x) * cos(0.5 * y)
	phi0 = [f(i, j) for i in 1:x_max, j in 1:y_max]
end;

# ╔═╡ 8d058c31-8973-4602-8706-88f4b4c9f4f5
let
	...
	heatmap!(rotr90(image), colormap=:grays)

	ax = CairoMakie.Axis(
		fig[1, 2],
		title = "Initial Level Set"
	)
	heatmap!(rotr90(phi0), colormap=:grays)

	fig
end

# ╔═╡ 88bd6113-b32f-48de-a4bc-cfd59eaedf08
md"""
## Convert to Symbolics
"""

# ╔═╡ 5f213ce4-49a5-48dd-9a38-16416e94c9b5
itp1 = interpolate(image, BSpline(Quadratic(Reflect(OnGrid()))));

# ╔═╡ 3d486418-af67-4731-83be-d1f8c4886aad
itp2 = interpolate(phi0, BSpline(Quadratic(Reflect(OnGrid()))));

# ╔═╡ d061f40b-96d4-4c25-b591-df663074cc9e
begin
	u⁰(x, y) = itp1(x, y)
	@register_symbolic u⁰(x, y) # symbolic & continuous shepp logan image (ℜ²)

	ϕ₀(x, y) = f(x, y)
	@register_symbolic ϕ₀(x, y) # symbolic & continuous level set at t=0 (ℜ²)
end

# ╔═╡ c3acbcb1-3360-41f6-999f-09b4846c0a9f
md"""
## Helper Equations
"""

# ╔═╡ f98569af-b09e-432e-84c4-dccbac397436
begin
	Ix = Integral(x in DomainSets.ClosedInterval(x_min, x)) # basically cumulative sum from 1 to x
	Iy = Integral(y in DomainSets.ClosedInterval(y_min, y)) # basically cumulative sum from 1 to y
end

# ╔═╡ c87bea2d-fd88-443b-8e6f-03e4e80d46eb
H = 1 / (1 + exp(-10 * ϕ(x, y, t)))

# ╔═╡ bd481ad6-e871-4357-9f1d-86caf7339e59
δ = Dt(H)

# ╔═╡ a3a72475-2bd1-4dce-b6d7-664950b4139a
c₁ = Ix(Iy(u⁰(x, y) * H)) / Ix(Iy(H))

# ╔═╡ 466fd8a6-8dff-42fa-b7c5-0580a2b15821
c₂ = Ix(Iy(u⁰(x, y) * (1 - H))) / Ix(Iy((1 - H)))

# ╔═╡ fff6554f-16dc-48ad-8e17-ed20621069c0
begin
...
# ╔═╡ a736cb4e-ebb4-4223-936d-6818b7208a82
md"""
## Set up PDE
"""

# ╔═╡ ddacf795-8712-411b-97ca-e0e711c3f580
const ν = 3

# ╔═╡ d87a1a11-6d5e-4edf-a71b-b0298158e9e7
eqs = [Dt(ϕ(x, y, t)) ~ δ * ((ν * div_ϕ) - (u⁰(x, y) - c₁)^2 + (u⁰(x, y) - c₂)^2)]

# ╔═╡ 00c10b7a-957f-4f01-aa4b-fcd3cfc99250
domains = [
	t ∈ Interval(0.0, 1.0),
...
# ╔═╡ a3696adc-3f7f-41bc-a595-a82fafad32b3
bcs = [
	ϕ(x, y, 0) ~ sin(0.5 * x) * cos(0.5 * y),
	ϕ(1, y, t) ~ ϕ(2, y, t),
	ϕ(x, 1, t) ~ ϕ(x, 2, t),
]

# ╔═╡ 5719b06d-342d-48c6-92be-c88f88004f18
@named pdesys = PDESystem(eqs, bcs, domains, [x, y, t], [ϕ(x, y, t)])

# ╔═╡ 2cac241d-4166-407b-851c-6deb3af47ebc
md"""
## Discretize
"""

# ╔═╡ 457a3161-0760-4736-8d64-c1bfc1f7f8df
discretization = MOLFiniteDifference([x => 30, y => 30], t)

# ╔═╡ ffd685b7-46a4-4e00-8825-f0b31954b4f0
prob = MethodOfLines.discretize(pdesys, discretization)

# ╔═╡ bfd4ffa5-25fe-429e-983b-73c46ec46dc9
sol = solve(prob);
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "e238c58ad7338c136ad29427a163625a50d1c4c4"
...
[[deps.MethodOfLines]]
deps = ["Combinatorics", "DiffEqBase", "DomainSets", "IfElse", "Interpolations", "Latexify", "LinearAlgebra", "ModelingToolkit", "OrdinaryDiffEq", "PDEBase", "SciMLBase", "StaticArrays", "SymbolicUtils", "Symbolics", "TermInterface"]
git-tree-sha1 = "3fe977a8d0de713fb95c37e533498488ebd839ef"
...
[[deps.PDEBase]]
deps = ["DomainSets", "ModelingToolkit", "SciMLBase", "SymbolicUtils", "Symbolics"]
git-tree-sha1 = "9eb9163847161e708d06289e64fbfe6ae14b7e96"

...
# ╔═╡ f58d0a34-e379-4bfa-b3f5-2c3acb19b94d
using Images, TestImages, PlutoUI, DifferentialEquations, Statistics, ForwardDiff, CairoMakie

# ╔═╡ 28036f23-bd9b-4b5c-9f7a-a86d87db216f
TableOfContents()

# ╔═╡ 149d7b47-973d-40bd-8aba-1ada8d29ab72
md"""
# Load Image
"""

# ╔═╡ 94c0b47a-624a-41c5-a9fb-8425c2291b7d
# img = TestImages.shepp_logan(256)

# ╔═╡ c7505aa2-b4e5-4f5e-b0f5-c230095eaa07
# ╠═╡ show_logs = false
# img = testimage("cameraman")

# ╔═╡ 3fe11c9c-da35-4507-82c3-cac1c40ba5e5
begin
	img = load("/Users/daleblack/Google Drive/dev/julia/ActiveContours/images/blobs.png")
	img = convert(Array{Float64}, Gray.(img))
end

# ╔═╡ b390ad99-7173-4b25-97b2-7afe09e1189f
md"""
# Write out Steps
"""

# ╔═╡ 0112e2c4-09c5-4bde-9d24-c6deb3c3c2b2
md"""
## 1. Level Set
Initialize the level set function with a simple closed curve that roughly encloses the object to be segmented. Turn this into a checkerboard instead.
"""

# ╔═╡ 43d2cfce-b062-441a-8acf-c81066d207cf
function init_level_set(img::AbstractArray, square_size = 5)
    width, height = size(img)
    # initialize level set function with all zeros
    level_set = zeros(size(img))

    # set all pixels inside the checkerboard squares to -1 and all pixels outside the squares to 1
    for x in 1:width, y in 1:height
        if (x % square_size <= square_size/2) == (y % square_size <= square_size/2)
            level_set[x, y] = -1
        else
            level_set[x, y] = 1
        end
...
# ╔═╡ a43e2f80-9bab-11ed-39e9-d1cb6d974068
level_set0 = init_level_set(img)

# ╔═╡ 06053800-5432-4b64-abd1-b60fe4baf150
Gray.(level_set0)

# ╔═╡ 7e2a31ba-2688-4279-a48d-0008090e4af9
md"""
## 2. Inside/Outside
Define the inside and outside regions of the curve based on the sign of the level set function.
"""

# ╔═╡ 7c12e9f8-af27-4229-bf09-d60b4ceb7689
function define_regions(level_set)
    # define inside and outside regions
    inside = level_set .< 0
    outside = level_set .> 0
...
# ╔═╡ 0cc3da02-ed7d-4bd7-8be7-5c9d172eb525
inside, outside = define_regions(level_set0)

# ╔═╡ 472cd7e0-3d28-496d-bac2-cdbe531cf4d7
Gray.(inside), Gray.(outside)

# ╔═╡ ea58a345-2b41-40b8-85b7-4bacfeb4ba66
md"""
## 3. Energy Functional
Define the energy functional, which consists of two terms: one that measures the fit of the level set function to the image data and another that measures the smoothness of the level set function.
"""

# ╔═╡ 4cb41c57-3551-4972-b96c-70a419c93c25
"""
	energy_functional(level_set, img, inside, outside)

This example is based on the energy functional function from the chan_vese function from scikit-image.
The energy_functional function takes 5 arguments, the level set level_set, the image img, the inside region inside, the outside region outside, the weight of the data term mu and the weight of the smoothness term nu.
The data_term is calculated by summing the square of the absolute difference between the pixel values inside the region and the mean pixel value inside the region and the difference between the pixel values outside the region and the mean pixel value outside the region.
The smoothness_term is calculated by summing the square of the absolute value of the gradient of the level set array.
The length_term is calculated by summing the absolute value of the gradient of the level set array.
The curvature_term is calculated by summing the square of the absolute value of the hessian of the level set array.
"""
function energy_functional(level_set, img, inside, outside, mu, nu)
    # data term
    data_term = sum(abs2, img .* inside .- mean(img[inside]) .- (img .* outside .- mean(img[outside])))
    # smoothness term
    dx,dy = imgradients(level_set, KernelFactors.ando3)
    smoothness_term = sum(abs2, dx) + sum(abs2, dy)
    # length term
    length_term = sum(abs, dx) + sum(abs, dy)
	
    # # curvature term
    # dxx, dxy, dyy = imhessian(level_set)
    # curvature_term = sum(abs2, dxx) + sum(abs2, dxy) + sum(abs2, dyy)
    
    return mu * data_term - nu * smoothness_term + length_term
end

# ╔═╡ 7fb738cd-a1ce-400b-8bfb-18ec07957624
E = energy_functional(level_set0, img, inside, outside, 1, 1)

# ╔═╡ 60828d46-9121-493f-9055-309b88e412b7
md"""
## 4. Minimize Energy Functional
Minimize the energy functional with respect to the level set function using gradient descent.
"""

# ╔═╡ 6843081a-9328-43b7-bf3c-d8bc670e5b3e
function gradient_E(level_set, img, inside, outside, mu, nu)
    # gradient of the data term
    gradient_data_term = 2 * (img .* inside .- mean(img[inside]) .- (img .* outside .- mean(img[outside]))) .* (inside - outside)
    # gradient of the smoothness term
    dx,dy = imgradients(level_set, KernelFactors.ando3)
    gradient_smoothness_term_x = -2 * nu * dx
    gradient_smoothness_term_y = -2 * nu * dy
    # gradient of the length term
    gradient_length_term_x = sign.(Float32.(dx))
    gradient_length_term_y = sign.(Float32.(dy))
    # # gradient of the curvature term
    # dxx, dxy, dyy = imhessian(level_set)
    # gradient_curvature_term_x = -2 * dxx
    # gradient_curvature_term_y = -2 * dyy
    # gradient_curvature_term_xy = -2 * dxy
    
    return mu * gradient_data_term + gradient_smoothness_term_x + gradient_smoothness_term_y + gradient_length_term_x + gradient_length_term_y
end

# ╔═╡ 6e2b7941-7a41-4846-8404-b12a18b7c08c
grad_E = gradient_E(level_set0, img, inside, outside, 1, 1)

# ╔═╡ 81a2b20e-7b2f-463d-ada8-a4154fa55c19
function minimize_energy(level_set, img, inside, outside, grad_E, learning_rate)
    # update the level set function
    level_set = level_set - learning_rate * grad_E

    return level_set
end

# ╔═╡ 1840696d-58ca-4880-8f1c-79b124c298a4
learning_rate = 1e-2

# ╔═╡ f9a9783a-af0e-4d3e-a640-6e4e5df6509d
md"""
## 5. Chan Vese
Repeat steps 2-4 until the level set function converges to a solution that segments the image.
"""

# ╔═╡ 7784ce17-1734-48f3-a3ea-335beb54f7c0
function chan_vese(level_set, mu=0.25, nu=1, max_iter = 2000, tolerance = 1e-6, prev_E = Inf)
	# repeat steps 2-4 until the level set function converges
	for iter in 1:max_iter
	    # define the inside and outside regions
	    inside, outside = define_regions(level_set)
	    
	    # calculate the energy functional
	    E = energy_functional(level_set, img, inside, outside, mu, nu)
	    ...
	    # calculate the gradient of the energy functional
	    grad_E = gradient_E(level_set, img, inside, outside, mu, nu)
	    
	    # minimize the energy functional
	    level_set = minimize_energy(level_set, img, inside, outside, grad_E, learning_rate)
	end
	return level_set
end

# ╔═╡ 022cd946-5e78-4c4c-80f9-ebd6ba7e0dea
level_set_final = chan_vese(level_set0)

# ╔═╡ 87030dfa-2e6d-40ba-a328-eaea7076cc9c
heatmap(level_set_final, colormap=:grays)

# ╔═╡ 336e3d93-d6cd-47fd-be98-279c89e384c9
heatmap(img, colormap=:grays)

# ╔═╡ 495a7b5e-456c-4711-b398-0dc960a80c3c
md"""
# Python chan vese approach
"""

# ╔═╡ 4ce6bb0d-1e4c-4aff-b98c-229f8b23e296
md"""
## Add functions
"""

# ╔═╡ 36f2103d-8c73-460c-a453-a9cdbc8cbf4c
md"""
### `_cv_checkerboard`
"""

# ╔═╡ 8a710852-c694-4830-a7b3-f13df1f9c9dc
function _cv_checkerboard(image_size, square_size)
...
# ╔═╡ f26bc693-552c-4223-bcb9-35ee232fcd86
checker = _cv_checkerboard(size(img), 5)

# ╔═╡ c83e0e21-0fa9-4456-bf92-ac5056834c34
heatmap(checker, colormap=:grays)

# ╔═╡ a4817d61-5861-431c-9151-682878bfd9ac
md"""
### `_cv_curvature`
"""

# ╔═╡ 282473b3-ac01-4d65-9a75-a4cd05f7f8c7


# ╔═╡ 392a3749-6fc8-49e2-acb0-f1c24944c4dd
...
# ╔═╡ 47c96964-2a62-40b7-a165-ea35ef3a0e33
test_phi = [
	10 -100 10 190 20
	10 -100 10 -100 20
	10 -100 10 -100 20
...
# ╔═╡ 17abbbfe-f547-4aab-ba84-38959e759e29
_cv_curvature(test_phi)

# ╔═╡ dc5ced33-3826-4bff-9ae4-a2918a428716
md"""
### `_cv_calculate_averages`
"""

# ╔═╡ 360b854a-f869-43c6-b2ff-25104f31fe5a
"""
...
# ╔═╡ afb929e9-6797-4e17-afcf-23cee6ff4d46
test_Hphi = 1 * (test_phi .> 0)

# ╔═╡ f1bcd33e-d881-433a-a43c-0514d4c33a46
test_image = [
	10 100 -10 0 10
	10 100 -10 0 10
	10 100 -10 0 10
...
# ╔═╡ 52ba4264-05cb-4a2b-9a2c-069ef6ffec99
_cv_calculate_averages(test_image, test_Hphi)

# ╔═╡ 1f63abbe-ce3c-4b61-ae0f-668df74333b7
md"""
### `_cv_delta`
"""

# ╔═╡ 495cb265-6e45-4bc8-9087-d6fcafae487f
function _cv_delta(x, eps=1.0)
    eps ./ (eps.^2 .+ x.^2)
end

# ╔═╡ 80358a4f-b696-4934-867c-ec88726adc40
_cv_delta(test_phi)

# ╔═╡ 590f14ee-035c-4eb7-bd2a-6421e3c6a5b9
md"""
### `_cv_calculate_variation`
"""

# ╔═╡ e97743dc-98b9-4897-9a18-5fd9a62362f9
"""
...
# ╔═╡ 161f7d31-970c-4513-92b6-55bfaaa01045
begin
	test_mu = 0.25
	test_lambda1 = 1
	test_lambda2 = 1
...
# ╔═╡ c4195fa4-d117-4463-ab9c-388f455c160c
_cv_calculate_variation(test_image, test_phi, test_mu, test_lambda1, test_lambda2, test_dt)

# ╔═╡ 965a311c-fae1-446d-ac93-249d8ce72fda
md"""
### `_cv_heavyside`
"""

# ╔═╡ 10ea6685-9a6e-48f4-8c23-e61b70ca22d5
"""
...
# ╔═╡ f35260ff-fc8f-4f09-ad05-94634760989a
_cv_heavyside(test_phi)

# ╔═╡ 8c265edb-d96c-4005-b05f-76823a13f048
md"""
### `_cv_difference_from_average_term`
"""

# ╔═╡ 9b96d6e1-2255-4a13-8a1a-be9eaecdb724
...
# ╔═╡ fb74c099-bed5-4f39-b357-6c8588cf2cb2
_cv_difference_from_average_term(test_image, test_Hphi, test_lambda1, test_lambda2)

# ╔═╡ a2ee94be-49fa-4fff-9532-e3cef6ffe652
md"""
### `_cv_edge_length_term`
"""

# ╔═╡ 5f988165-3d58-4d7b-9477-bac72fde66c3
function _cv_edge_length_term(phi, mu)
    toret = _cv_curvature(phi)
    return mu * toret
end

# ╔═╡ 0155504f-269c-42ab-b856-c2c1840ae8e1
_cv_edge_length_term(test_phi, test_mu)

# ╔═╡ 3449c81a-38b5-474c-9774-86fa37ca94ae
md"""
### `_cv_energy`
"""

# ╔═╡ 58be1620-5a73-4d48-a4ee-4bcacdbfe9c4
...
# ╔═╡ a973f27c-78eb-4b69-9bd2-e24631484f61
_cv_energy(test_image, test_phi, test_mu, test_lambda1, test_lambda2)

# ╔═╡ 37a81823-f6d1-4445-9354-aed170d2955b
md"""
## Run loop
"""

# ╔═╡ 687c4ab4-00dc-4211-8d71-a5a175d3c0e3
begin
	# hardcode parameters
	mu = 0.25
	lambda1 = 1
...
# ╔═╡ dcc14f77-961a-4ab6-b498-cb668ef4071f
function chan_vese2(image; mu=0.25, lambda1=1, lambda2=1, tol=1e-3, max_iter=500, dt=0.5)
    phi = _cv_checkerboard(size(image), 5)
...
# ╔═╡ c3e9a15c-a475-4168-bbd3-f52bd1303e61
segmentation, phi, energies = chan_vese2(img; max_iter=200);

# ╔═╡ b078a7cc-aaa0-4cd5-a145-42cf75d59556
let
	f = Figure()
...
# ╔═╡ 136ea760-ce0f-44c4-975e-06ea80de5a51


# ╔═╡ d83572a6-c38b-4500-bbed-cb6c759f9ee1
md"""
# SciML UDE Approach
```julia
using Pkg
...
# Step 1: Install the SciML packages
Pkg.add("SciML")
...
Flux.train!(loss, Flux.data(train_data), optimizer=Adam(0.01))

# Step 3: Incorporate the neural network into the energy functional
function energy_functional(level_set, img, inside, outside, model)
    # data term
    data_term = sum(abs2, img .* inside - mean(img[inside]) .- (img .* outside - mean(img[outside])))
    # smoothness term
    smoothness_term = sum(abs2, gradient(level_set))
    # neural network term
    neural_network_term = sum(abs2, model(img) .* level_set)
    
    return data_term + smoothness_term + neural_network_term
end

# Step 4: Modify the gradient of the energy functional
function gradient_E(level_set, img, inside, outside, model)
    # gradient of the data term
    gradient_data_term = 2 * (img .* inside - mean(img[inside]) .- (img .* outside - mean(img[outside]))) .* (inside - outside)
    # gradient of the smoothness term
    gradient_smoothness_term = 2 * gradient(level_set, centered=false)
    # gradient of the neural network term
    gradient_neural_network_term = 2 * Tracker.jacobian(model(img), level_set) .* level_set
    
    return gradient_data_term + gradient_smoothness_term + gradient_neural_network_term
end

# Step 5: Modify the level set evolution equation
function level_set_evolution(du, u, p, t)
    level_set = u
    img, inside, outside, model, learning_rate = p
    grad_E = gradient_E(level_set, img, inside, outside, model)
    du .= -learning_rate * grad_E
    Tracker.back!(grad_E, model)
end

# Step 6: Solve the modified level set evolution equation
# Define the initial condition and boundary conditions
u0 = init_level_set(img)
tspan = (0.0, 1.0)

# Define the ODE problem
prob = ODEProblem(level_set_evolution, u0, tspan, img, inside, outside, model)

# Solve the ODE problem
solution = solve(prob)

# Step 7: Integrate the neural network into the level set evolution
# Train the neural network during the level set evolution
for i in 1:num_iterations
    # Update the parameters of the neural network
    Flux.train!(loss, Flux.data(img), optimizer=Adam(0.01))
    # Update the level set function
    solution = solve(prob, solution.u)
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "58479309fe5ec79b98ec5bf88adffbd4e83002ba"
